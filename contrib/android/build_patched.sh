...
# Fix gradle permissions for local builds too
if [ -z "$ELECBUILD_COMMIT" ] ; then
    if [ $(id -u) != "1000" ] || [ $(id -g) != "1000" ] ; then
        info "need to chown -R gradle dir. prompting for sudo."
        sudo chown -R 1000:1000 "$PROJECT_ROOT_OR_FRESHCLONE_ROOT"/.buildozer/.gradle 2>/dev/null || true
        sudo chown -R 1000:1000 "$PROJECT_ROOT_OR_FRESHCLONE_ROOT"/.buildozer 2>/dev/null || true
    fi
fi
docker run --rm \
    ...
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT"/.buildozer:/home/user/.buildozer \
    $DOCKER_RUN_FLAGS \
